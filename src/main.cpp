...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 2000;	// stake age of full weight: 2000d
unsigned int nStakeTargetSpacing = 60;			// 60 sec block spacing

int64 nChainStartTime = 1400899612;
...
const int DAILY_BLOCKCOUNT =  1440;	    //  one day
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
		...
		if(pindexBest->nHeight < (1 * 365 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 100 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (2 * 365 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 80 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (3 * 365 * DAILY_BLOCKCOUNT))
  		  nRewardCoinYear = 60 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (4 * 365 * DAILY_BLOCKCOUNT))
  		  nRewardCoinYear = 40 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (5 * 365 * DAILY_BLOCKCOUNT))
  		  nRewardCoinYear = 20 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (6 * 365 * DAILY_BLOCKCOUNT))
  		  nRewardCoinYear = 10 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (7 * 365 * DAILY_BLOCKCOUNT))
  		  nRewardCoinYear = 5 * MAX_MINT_PROOF_OF_STAKE;
  	else if(pindexBest->nHeight < (8 * 365 * DAILY_BLOCKCOUNT))
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TradeCoin
    bool fStrictPayToScriptHash = true; // Always active in TradeCoin
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "NOW , LET US DO IT.2014 05 24";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1400899622;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3957920;
       ...
       //   }
        ...
        assert(block.hashMerkleRoot == uint256("0x69a6cd7790c67cd1a70257cc20f971ff2e6f3159f370a3e076ada24929618ca0"));
...
unsigned char pchMessageStart[4] = { 0xcd, 0xfb, 0xfb, 0xd9 };
